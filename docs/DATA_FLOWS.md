...
| Firecrawl research index (`scripts/firecrawl_client.py`) | **Callable client only (no CLI wrapper) — deliberately NOT wired into the four-index verification gate** (a ranked semantic index cannot supply the gate's `*_unmatched` boolean; see the protocol doc's "Where this stops") | Source-namespaced ids (`arxiv:`/`pmid:`/`pmcid:`) or title query strings of the works you look up; on the passages path additionally **the question text you ask of a paper**, which can derive from unpublished draft claims | `api.firecrawl.dev` | none required — both endpoints answer keyless; `FIRECRAWL_API_KEY` is optional and only raises the rate limit (sent as an `Authorization` header, never in the URL) | Don't call the client |
